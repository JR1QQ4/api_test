...
class TestUserManage:
    ...
    @pytest.mark.parametrize('case', HandleExcel(wx, "更新成员").get_all_data())
    def test_add_user(self, case, token):
        ...
        if res_json['errcode'] == code and res_json['errmsg'] == msg:
            HandleExcel(self.wx, "更新成员").update(case, "pass")
        else:
            HandleExcel(self.wx, "更新成员").update(case, res_json)


if __name__ == '__main__':
    pytest.main(["-sv", "test_user_manage_update.py"])
